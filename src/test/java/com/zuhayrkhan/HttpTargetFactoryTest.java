...
class HttpTargetFactoryTest {
    ...
    public static Stream<HttpTargetTestParams<SimpleMapContext>>
    createURIStringsAndExpectedMapStrategy() {

        HttpTargetFactory<SimpleMapContext> httpTargetFactory =
                new HttpTargetFactory<>(
                        new SimpleExpressionConverter<>(CLOCK, new MapConverterStrategy()));

        return Stream.of(
                httpTargetTestParamsWithSomeConstAmongVars(SimpleMapContext.class)
                        .withHttpTargetFactory(httpTargetFactory).build(),
                httpTargetTestParamsWithSomeVars(SimpleMapContext.class)
                        .withHttpTargetFactory(httpTargetFactory).build(),
                httpTargetTestParamsWithNoVars(SimpleMapContext.class)
                        .withHttpTargetFactory(httpTargetFactory).build()
        );
    }

    public static Stream<HttpTargetTestParams<JexlContext>>
    createURIStringsAndExpectedJexlStrategy() {

        HttpTargetFactory<JexlContext> httpTargetFactory =
                new HttpTargetFactory<>(new SimpleExpressionConverter<>(CLOCK, new JexlConverterStrategy()));

        return Stream.of(
                httpTargetTestParamsWithSomeConstAmongVars(JexlContext.class)
                        .withHttpTargetFactory(httpTargetFactory).build(),
                httpTargetTestParamsWithSomeVars(JexlContext.class)
                        .withHttpTargetFactory(httpTargetFactory).build(),
                httpTargetTestParamsWithNoVars(JexlContext.class)
                        .withHttpTargetFactory(httpTargetFactory).build()
        );
    }
